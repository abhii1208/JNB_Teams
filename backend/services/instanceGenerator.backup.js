...
async function generateInstancesForSeries(seriesId, options = {}) {
    ...
    try {
        ...
        // Using pg_try_advisory_xact_lock instead of pg_try_advisory_lock
        // to prevent lock persistence if previous job crashed
        const lockResult = await client.query(
            'SELECT pg_try_advisory_xact_lock($1)',
            [seriesId]
        );

        lockAcquired = Boolean(lockResult.rows[0]?.pg_try_advisory_xact_lock);
        if (!lockAcquired) {
        ...
        // Use look_ahead_days if available, otherwise fall back to prevent_future setting
        const lookAheadDays = series.look_ahead_days ?? 1; // Default: generate today + tomorrow
        const shouldPreventFuture = preventFuture !== null 
            ? preventFuture 
            : (series.prevent_future ?? false); // Changed default to FALSE
        
        // Calculate the maximum future date we should generate up to
        const maxFutureDate = DateTime.now().setZone(timezone).plus({ days: lookAheadDays }).toISODate();
        
        // FIX: Limit backfill to reasonable range (max 30 days back by default)
        // This prevents generating hundreds of past instances when start_date is far in the past
        const maxBackfillDays = 30;
        const earliestBackfillDate = DateTime.now().setZone(timezone).minus({ days: maxBackfillDays }).toISODate();

        // FIXED: Simplified early exit - only skip if we have enough future tasks
        // AND we've already generated for today (prevents skipping when no tasks exist)
        const hasTodayTask = await client.query(`
            SELECT 1 FROM tasks 
            WHERE series_id = $1 AND occurrence_date = $2 AND deleted_at IS NULL
            LIMIT 1
        `, [seriesId, today]);
        ...
        if (
            ...
            && !forceRegenerate
            && !shouldBackfill
            && hasTodayTask.rows.length > 0 // Must have today's task
        ) {
        ...
        // FIX: Respect the backfill limit - don't go further back than maxBackfillDays
        let startPoint = lastGeneratedAt && totalCount > 0
            ? lastGeneratedAt
            : DateTime.fromISO(seriesStartDate, { zone: timezone })
                .minus({ days: 1 })
                .toISODate();
        
        // Clamp startPoint to not be earlier than the backfill limit
        if (shouldBackfill && startPoint < earliestBackfillDate) {
            startPoint = earliestBackfillDate;
        }
        ...
        let iterations = 0;
        const canGenerateFuture = (checkDate) => {
            // FIXED: Use look_ahead_days for cleaner future generation control
            // If prevent future is enabled AND we're beyond look-ahead window, stop
            if (shouldPreventFuture && checkDate > maxFutureDate) return false;
            // Also respect the max future instances limit
        ...
        while (
            ...
            // FIXED: Check if we can generate this future date using look_ahead_days
            if (isFuture && !canGenerateFuture(nextDate)) {
            ...
            if (exception) {
                if (exception.exception_type === 'skip') {
                    ...
                    // FIXED: Use canGenerateFuture with date parameter
                    if (movedIsFuture && !canGenerateFuture(movedDate)) {
                        break;
                    }
        ...
        // Update last_generated_at and clear any retry state on success
        if (generatedCount > 0) {
            await client.query(`
            ...
            // Even if nothing generated, clear error state
            await client.query(`
                UPDATE recurring_series 
                SET last_generation_error = NULL,
                    generation_retry_count = 0,
                    next_retry_at = NULL
                WHERE id = $1
            `, [seriesId]);
        ...
        // HIGH PRIORITY FIX: Track errors and schedule retry
        try {
            const retryClient = await pool.connect();
            try {
                // Exponential backoff: 5min, 15min, 45min, 2hr, 6hr
                const retryDelays = [5, 15, 45, 120, 360];
                await retryClient.query(`
                    UPDATE recurring_series 
                    SET last_generation_error = $2,
                        generation_retry_count = COALESCE(generation_retry_count, 0) + 1,
                        next_retry_at = NOW() + (($3)[LEAST(COALESCE(generation_retry_count, 0) + 1, 5)] || ' minutes')::INTERVAL
                    WHERE id = $1
                `, [seriesId, error.message.substring(0, 500), retryDelays]);
            } finally {
                retryClient.release();
            }
        } catch (retryError) {
            console.error('Failed to update retry state for series', seriesId, retryError);
        }
        
        return result;
    } finally {
        // Note: pg_try_advisory_xact_lock auto-releases on transaction end
        // No need to manually unlock - this prevents lock persistence on crashes
        client.release();
    }
}
 ...
 * HIGH PRIORITY FIX: Now includes retry queue processing
 */
async function generateAllPendingInstances(options = {}) {
    const { batchSize = 50, lookAheadDays = 1, todayOnly = false } = options;
    
    // Build query to include both regular series and retry queue
    const result = await pool.query(`
        SELECT id, 
               CASE WHEN next_retry_at IS NOT NULL AND next_retry_at <= NOW() THEN 'retry' ELSE 'normal' END as queue_type
        FROM recurring_series
        WHERE deleted_at IS NULL
        AND paused_at IS NULL
        AND (end_date IS NULL OR end_date >= CURRENT_DATE)
        AND (
            -- Normal queue: not in retry state or retry is due
            next_retry_at IS NULL 
            OR next_retry_at <= NOW()
        )
        -- Prioritize retries, then series that haven't been generated recently
        ORDER BY 
            CASE WHEN next_retry_at IS NOT NULL AND next_retry_at <= NOW() THEN 0 ELSE 1 END,
    ...
    for (const row of result.rows) {
        const genOptions = {
            ...options,
            // For todayOnly mode, prevent future generation beyond today
            preventFuture: todayOnly ? true : (options.preventFuture ?? null)
        };
        
        const genResult = await generateInstancesForSeries(row.id, genOptions);
        summary.processed++;
        summary.totalGenerated += genResult.generated;
        summary.totalSkipped += genResult.skipped;
        
        if (row.queue_type === 'retry') {
            summary.retried++;
        }
